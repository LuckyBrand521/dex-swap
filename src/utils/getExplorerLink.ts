...
const BASESCAN_PREFIXES: { [chainId: number]: string } = {
  [SupportedChainId.BASE_GOERLI]: 'goerli.',
}
...
export function getExplorerLink(chainId: number, data: string, type: ExplorerDataType): string {
  ...
  if (chainId === SupportedChainId.BASE_GOERLI) {
    const prefix = `https://${BASESCAN_PREFIXES[chainId] ?? ''}basescan.org`
