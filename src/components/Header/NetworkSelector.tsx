...
const SelectorControls = styled.div<{ interactive: boolean }>`
  ...
  justify-content: space-between;
  padding: 6px 8px;
  box-shadow: inset 0px 0px 15px #5266FF;
  &::before{
    content: "";
    position: absolute;
    inset: 0;
    border-radius: 5px; 
    padding: 1px; 
    background: linear-gradient(91.82deg, #7AD0E3 2.57%, #5266FF 99.22%);
    -webkit-mask: 
...
export default function NetworkSelector() {
  ...
  const showSelector = true || Boolean(implements3085 || isOnL2)
  ...
  function Row({ targetChain }: { targetChain: number }) {
    ...
    const isBase = targetChain === SupportedChainId.BASE_GOERLI
    ...
    const helpCenterLink = isOptimism ? OPTIMISM_HELP_CENTER_LINK : isBase ? BASE_HELP_CENTER_LINK : ARBITRUM_HELP_CENTER_LINK
    ...
    <SelectorWrapper ref={node as any}>
      ...
      {open && (
        <FlyoutMenu>
          ...
          <Row targetChain={SupportedChainId.BASE_GOERLI} />
